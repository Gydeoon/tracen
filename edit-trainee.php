...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $query = "UPDATE trainees SET 
                ...
                stat_speed = :spd, stat_stamina = :sta, stat_power = :pow, stat_guts = :gut, stat_wit = :wit, 
                final_rank = :rank, 
                rating_score = :score, 
                notes = :notes 
              WHERE id = :id";
    ...
    $_SESSION['message'] = "Training Data Successfully Updated!";
...
<head>
    ...
    <title>Edit Trainee | Tracen Academy</title>
...
<body class="antialiased min-h-screen py-10 flex items-center justify-center">
    ...
    <div class="w-full max-w-5xl bg-white p-8 md:p-12 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100">
        ...
        <div class="flex justify-between items-center mb-10 pb-6 border-b border-gray-100">
            <div><h2 class="text-3xl font-black text-gray-900 uppercase italic tracking-tight">Edit <span class="uma-primary-text">Trainee</span></h2></div>
        ...
        <form action="edit-trainee.php" method="POST" class="flex flex-col md:flex-row gap-10">
            ...
            <div class="w-full md:w-2/3 space-y-6">
                <div>
                    ...
                    <select name="character_id" id="charSelect" required class="clean-input font-bold text-gray-800 cursor-pointer">
                        ...
                        <?php foreach($characters as $c): ?>
                            <option value="<?php echo $c['id']; ?>" <?php if($c['id'] == $trainee['character_id']) echo 'selected'; ?>>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-5 gap-3">
                        <input type="number" name="stat_speed" value="<?php echo $trainee['stat_speed']; ?>" class="clean-input text-center font-bold !px-1 text-blue-600 focus:!border-blue-500" required>
                        <input type="number" name="stat_stamina" value="<?php echo $trainee['stat_stamina']; ?>" class="clean-input text-center font-bold !px-1 text-pink-600 focus:!border-pink-500" required>
                        <input type="number" name="stat_power" value="<?php echo $trainee['stat_power']; ?>" class="clean-input text-center font-bold !px-1 text-orange-600 focus:!border-orange-500" required>
                        <input type="number" name="stat_guts" value="<?php echo $trainee['stat_guts']; ?>" class="clean-input text-center font-bold !px-1 text-red-600 focus:!border-red-500" required>
                        <input type="number" name="stat_wit" value="<?php echo $trainee['stat_wit']; ?>" class="clean-input text-center font-bold !px-1 text-green-600 focus:!border-green-500" required>
                    </div>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 bg-gray-50 p-4 rounded-2xl border border-gray-100">
                    <div class="md:col-span-1">
                        <label class="block text-xs font-black text-gray-400 mb-2 uppercase tracking-wide">Update Score</label>
                        <input type="number" name="rating_score" id="ratingInput" value="<?php echo htmlspecialchars($trainee['rating_score'] ?? ''); ?>" placeholder="Score" class="clean-input font-black text-gray-800" required>
                    </div>
                    
                    <div class="md:col-span-1">
                        <label class="block text-xs font-black text-gray-400 mb-2 uppercase tracking-wide text-center">Rank</label>
                        <input type="hidden" name="final_rank" id="finalRankInput" value="<?php echo htmlspecialchars($trainee['final_rank']); ?>" required>
                        <div id="rankDisplay" class="bg-gray-200 text-gray-400 font-black h-[46px] rounded-xl flex items-center justify-center text-xl transition-colors shadow-inner">
                            <?php echo htmlspecialchars($trainee['final_rank']); ?>
                        </div>
                    </div>
                    
                    <div class="md:col-span-2">
                        <label class="block text-xs font-black text-gray-400 mb-2 uppercase tracking-wide">Trainer Notes</label>
                        <input type="text" name="notes" value="<?php echo htmlspecialchars($trainee['notes']); ?>" class="clean-input text-gray-700 font-medium">
                    </div>
                </div>
                
                <button type="submit" class="w-full bg-gray-800 text-white font-black py-4 rounded-xl hover:scale-[1.02] transition-transform shadow-lg shadow-gray-200 mt-4 text-lg tracking-wider">
                    UPDATE DATABASE
                </button>
            </div>
        </form>
    ...
    <script>
        ...
        window.addEventListener('DOMContentLoaded', () => {
            ...
            if (ratingInput.value) { ratingInput.dispatchEvent(new Event('input')); }
        });
    </script>
</body>
</html>